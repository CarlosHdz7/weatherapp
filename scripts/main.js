...
const displayCityData = async objCityInfo => {
  ...
  lblSunrise.textContent = new Date(objCityInfo.sun_rise).toLocaleString(
    navigator.language,
    { hour: '2-digit', minute: '2-digit' }
  );
  lblSunset.textContent = new Date(objCityInfo.sun_set).toLocaleString(
    navigator.language,
    { hour: '2-digit', minute: '2-digit' }
  );
...
const createDayCard = day => {
  ...
  const text = document.createTextNode(
    new Date(`${day.applicable_date} 00:00`).toLocaleString('en-us', {
      weekday: 'long',
    })
  );
  ...
  img.setAttribute(
    'src',
    `${BASEURLMETA}/static/img/weather/${day.weather_state_abbr}.svg`
  );
